...
using the commonly used ADMIXTURE program as well as the python package `fastmixture`.
...
scales making it a more scalable option for large genetic data sets.
...
from clustering analyses (irrespective of the algorithm used) can be easily
visualised using the `gt_clumppling` 
function, which allows for easy alignment and comparison of multiple clustering 
results (uusing the clumppling python package (<https://github.com/PopGenClustering/Clumppling>)
(Liu, Kopelman, & Rosenberg, 2024).

Genetic data analyses through the integration of `tidypopgen` and `tidygenclust`
seamlessly joins upstream genetic data manipulation (`tidypopgen`)
with downstream clustering analyses and extensive visualisation 
options of the clustering results (`tidygenclust`) in an integrated pipeline
