...
test_that("gt_fastmixture", {
  ...
  anole_qmat <- gt_fastmixture(anole_gt, k, n_runs=1, threads=1, seed=42,
                                   iter=1000, tole=0.5,
  ...
  anole_qmat <- gt_fastmixture(anole_gt, k, n_runs=2, threads=1, seed=c(42,31),
                               iter=1000, tole=0.5,
                               batches=32, supervised=NULL, check=5, power=11,
                               chunk=8192, als_iter=1000, als_tole=1e-4,
                               no_freqs=FALSE, random_init=TRUE)
  #correct structure
  expect_true(inherits(anole_qmat,c("list", "gt_admix")))
  expect_true(all(names(anole_qmat) == names(anole_qmat)))
  expect_true(inherits(anole_qmat$Q[[1]],"q_matrix"))
